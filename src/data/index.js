const cloud9Image = '/images/cloud9.png'
const creemaImage = '/images/creema.png'
const minneImage = '/images/minne.png'
const instagramImage = '/images/instagram.png'
const twitterImage = '/images/twitter.jpeg'
const purpleheartImage = '/images/purpleheart.png'
const satineImage = '/images/satine.png'
const karinImage = '/images/karin.png'
const pinkivoryImage = '/images/pinkivory.png'
const snakewoodImage = '/images/snakewood.png'
const mapleImage = '/images/maple.png'
const wengeImage = '/images/wenge.png'
const rosewoodImage = '/images/rosewood.png'
const padaukImage = '/images/padauk.png'
const crystalEbonyImage = 'images/crystalEbony.png'
const crystalKarinImage = 'images/crystalKarin.png'
const crystalRingImage = 'images/crystalRing.JPG'

export const shop = {
  url: 'https://www.creema.jp/creator/7614629/item/onsale',
  image: cloud9Image,
}

export const links = [
  {
    id: 'creema',
    url: 'https://www.creema.jp/creator/7614629/item/onsale',
    image: creemaImage,
    name: 'creema',
  },
  {
    id: 'minne',
    url: 'https://minne.com/@cloud9-wood',
    image: minneImage,
    name: 'minne',
  },
  {
    id: 'instagram',
    url: 'https://instagram.com/cloud9_woodwork?igshid=YmMyMTA2M2Y=',
    image: instagramImage,
    name: 'Instagram',
  },
  {
    id: 'twitter',
    url: 'https://twitter.com/cloud9_woodwork?s=21&t=bdwddBl2tEFpUtr9tMjI5w',
    image: twitterImage,
    name: 'Twitter',
  },
]

export const products = [
  {
    id: 'purpleheart',
    url: 'https://www.creema.jp/item/14943547/detail',
    mUrl: 'https://minne.com/items/34692068',
    image: purpleheartImage,
    enName: 'cloud9 WoodRing PURPLE HEART',
    jpName: 'cloud9 ウッドリング パープルハート',
    intro: '「信頼」や「確実性」の意味をともなうパープルカラーの指輪。指に取り入れるだけでいつもより大人な雰囲気に包まれます。',
    name: 'パープルハート',
  },
  {
    id: 'satine',
    url: 'https://www.creema.jp/item/14943485/detail',
    mUrl: 'https://minne.com/items/34692037',
    image: satineImage,
    enName: 'cloud9 WoodRing SATINE',
    jpName: 'cloud9 ウッドリング サティーネ',
    intro: '「深紅の赤」と言われる見れば見るほど美しい指輪。金色の筋が現れ、見る方向によって色合いと光沢の変化が感じられます。',
    name: 'サティーネ',
  },
  {
    id: 'karin',
    url: 'https://www.creema.jp/item/14943464/detail',
    mUrl: 'https://minne.com/items/34692007',
    image: karinImage,
    enName: 'cloud9 WoodRing KARIN',
    jpName: 'cloud9 ウッドリング カリン',
    intro: 'とても木肌が美しく、綺麗な光沢を持った指輪。男性にも女性にも、年齢問わず幅広く合わせることができます。ペアリングにもオススメです。',
    name: '花梨',
  },
  {
    id: 'pinkivory',
    url: 'https://www.creema.jp/item/14943315/detail',
    mUrl: 'https://minne.com/items/34691963',
    image: pinkivoryImage,
    enName: 'cloud9 WoodRing PINK IVORY',
    jpName: 'cloud9 ウッドリング ピンクアイボリー',
    intro: '宝石のように非常に高価で希少なピンクアイボリー。天然色とは思えないほど綺麗なピンク色をしており、”木材の女王”と言われています。',
    name: 'ピンクアイボリー',
  },
  {
    id: 'snakewood',
    url: 'https://www.creema.jp/item/14943228/detail',
    mUrl: 'https://minne.com/items/34691875',
    enName: 'cloud9 WoodRing SNAKE WOOD',
    jpName: 'cloud9 ウッドリング スネークウッド',
    intro: '木材の”王様”スネークウッド。男性的で力強い指輪は、着けることで圧倒的な格好良さと色気で他人と差をつけることができます。',
    image: snakewoodImage,
    name: 'スネークウッド',
  },
  {
    id: 'maple',
    url: 'https://www.creema.jp/item/14943075/detail',
    mUrl: 'https://minne.com/items/34691841',
    image: mapleImage,
    enName: 'cloud9 WoodRing MAPLE',
    jpName: 'cloud9 ウッドリング メープル',
    intro: '白く真珠のような輝きを放つメープル。白いメープルの指輪を着けることで、純白で汚れのない…そんな印象を与えることができます。',
    name: 'メープル',
  },
  {
    id: 'wenge',
    url: 'https://www.creema.jp/item/14943033/detail',
    mUrl: 'https://minne.com/items/34691768',
    image: wengeImage,
    enName: 'cloud9 WoodRing WENGE',
    jpName: 'cloud9 ウッドリング ウェンジ',
    intro: '黒く落ち着いた色調と独特で繊細な木目が美しいウェンジ。そんな指輪を着けるとシックで大人な印象に仕立ててくれます。',
    name: 'ウェンジ',
  },
  {
    id: 'rosewood',
    url: 'https://www.creema.jp/item/14942931/detail',
    mUrl: 'https://minne.com/items/34691693',
    image: rosewoodImage,
    enName: 'cloud9 WoodRing ROSE WOOD',
    jpName: 'cloud9 ウッドリング ローズウッド',
    intro: '古くから高級家具等で使用されていたローズウッド。黒紫色で美しい最高級の指輪を着けて日々をいつも以上に楽しくお過ごしください。',
    name: 'ローズウッド',
  },
  {
    id: 'padauk',
    url: 'https://www.creema.jp/item/14942878/detail',
    mUrl: 'https://minne.com/items/34513722',
    image: padaukImage,
    enName: 'cloud9 WoodRing PADAUK',
    jpName: 'cloud9 ウッドリング パドック',
    intro: '赤く美しいパドック。繊細だけどクールな赤、明るく前向きで情熱的な赤。そんなあなたの気持ちと共に寄り添ってくれる指輪です。',
    name: 'パドック',
  },
]
export const productsCrystal = [
  {
    id: 'crystalEbony',
    url: 'https://www.creema.jp/item/15236609/detail',
    mUrl: 'https://minne.com/items/35274325',
    image: crystalEbonyImage,
    enName: 'cloud9 Crystal&WoodRing Ebony',
    jpName: 'cloud9 クリスタル&ウッドリング エボニー',
    intro: 'クリスタルを埋め込んだウッドリングになります。『単色だと物足りなかった』『手元にはやっぱりキラキラした指輪をつけたい』そんなあなたのための指輪です。',
    name: 'クリスタルエボニー',
  },
  {
    id: 'crystalKarin',
    url: 'https://www.creema.jp/item/15236735/detail',
    mUrl: 'https://minne.com/items/34692090',
    image: crystalKarinImage,
    enName: 'cloud9 Crystal&WoodRing Karin',
    jpName: 'cloud9 クリスタル&ウッドリング カリン',
    intro: 'クリスタルを埋め込んだウッドリングになります。『単色だと物足りなかった』『手元にはやっぱりキラキラした指輪をつけたい』そんなあなたのための指輪です。',
    name: 'クリスタルカリン',
  },
]
export const gallaryCrystal = [
  {
    id: 'crystalKarin',
    url: 'https://www.creema.jp/item/15236735/detail',
    mUrl: 'https://minne.com/items/34692090',
    image: crystalKarinImage,
    enName: 'cloud9 Crystal&WoodRing Karin',
    jpName: 'cloud9 クリスタル&ウッドリング カリン',
    intro: 'クリスタルを埋め込んだウッドリングになります。『単色だと物足りなかった』『手元にはやっぱりキラキラした指輪をつけたい』そんなあなたのための指輪です。',
    name: 'クリスタルカリン',
  },
  {
    id: 'crystalRing',
    url: 'https://www.creema.jp/item/15236735/detail',
    mUrl: 'https://minne.com/items/34692090',
    image: crystalRingImage,
    enName: 'cloud9 Crystal&WoodRing',
    jpName: 'cloud9 クリスタル&ウッドリング',
    intro: 'クリスタルを埋め込んだウッドリングになります。『単色だと物足りなかった』『手元にはやっぱりキラキラした指輪をつけたい』そんなあなたのための指輪です。',
    name: 'クリスタルリング',
  },
  {
    id: 'crystalEbony',
    url: 'https://www.creema.jp/item/15236609/detail',
    mUrl: 'https://minne.com/items/35274325',
    image: crystalEbonyImage,
    enName: 'cloud9 Crystal&WoodRing Ebony',
    jpName: 'cloud9 クリスタル&ウッドリング エボニー',
    intro: 'クリスタルを埋め込んだウッドリングになります。『単色だと物足りなかった』『手元にはやっぱりキラキラした指輪をつけたい』そんなあなたのための指輪です。',
    name: 'クリスタルエボニー',
  },

]
